...
function searchQueries(basePath, searchText) {
  return fetch(`${basePath}api/queries/search?q=${searchText}`)
    .then(r => r.json())
    .then(qs => qs.map(q => ({ value: q.id, label: q.name })));
}
    ...
    extraField = (
      <div className="form-group">
        ...
        <Select.Async
          ...
          loadOptions={searchText => (searchText.length > 3 ? searchQueries(clientConfig.basePath, searchText) : null)}
        />
      </div>);
  }
  return (
